...
@implementation AliPaySuccess
{
    CALayer *_animationLayer;
}
+ (void)successShowIn:(UIView *)view {
    AliPaySuccess *hud = [[AliPaySuccess alloc]initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
}
+ (void)successHideIn:(UIView *)view {
    AliPaySuccess *hud = nil;
    for (AliPaySuccess *subview in view.subviews) {
        if ([subview isKindOfClass:[AliPaySuccess class]]) {
            [subview hide];
...
- (void)start {
    ...
    dispatch_time_t time = dispatch_time(DISPATCH_TIME_NOW, 5 * circleDuration * NSEC_PER_SEC);
...
- (void)circleAniamtion {
    ...
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path addArcWithCenter:circleLayer.position radius:radius startAngle:-M_PI_2 endAngle:M_PI*3/2 clockwise:true];
    circleLayer.path = path.CGPath;
    
    CABasicAnimation *circleAnimation = [CABasicAnimation animationWithKeyPath:@"strokeEnd"];
    circleAnimation.duration = circleDuration*5;
